...
DATA_SPLIT = {
    ...
    "wcci": (  # train, val, test split chronics
        [
            17,
            240,
            494,
...
def read_ffw_json(path, chronics, case):
    res = {}
    for i in chronics:
        for j in range(MAX_FFW[case]):  # for each chronics, try all fastforwards (0,25)
            with open(
                os.path.join(path, "%d_%d.json" % (i, j)), "r", encoding="utf-8"
            ) as f:  # chronic_fastforward
                a = json.load(f)
                res[(i, j)] = (
                    a["dn_played"],
                    a["donothing_reward"],
...
if __name__ == "__main__":
    ...
    dn_json_path = os.path.join(env_path, "json")  # do nothing json

    # select chronics
    dn_ffw = read_ffw_json(dn_json_path, test_chronics, args.case)
    # dictionary (chronic, ffw) : (do_nothing_played, reward, no_disc_reward)

    # when small number of train, test chronic
    ep_infos = {}
    # dictionary [chronic]['length', 'dn_played', 'donothing_reward', 'donothing_nodisc_reward', 'sum_loads', 'losses', 'std_reward', 'loss_reward', 'redisp_reward']
