...
class Agent(BaseAgent):
    def __init__(self, experiment, env, **kwargs):
        ...
        self.converter = graphGoalConverter(
            env, mask, mask_hi, self.danger, self.device, self.rule
        )
        ...
        self.state_dim = kwargs.get("state_dim", 128)  # k_s is 128
        ...
        self.emb = EncoderLayer(
            self.input_dim, self.state_dim, self.nheads, self.node_num, self.dropout
        ).to(self.device)
        ...
        self.target_entropy = (
            ...
            else -3 * (self.action_dim + self.order_dim)
        )
    ...
    def stack_obs(self, obs):
        obs_vect = obs.to_vect()  # from grid2op, gives a bunch of info
        ...
        if len(self.stacked_obs) == 0:
            ...
            self.stacked_obs.append(obs_vect)
        self.adj = (
            torch.FloatTensor(obs.connectivity_matrix()) + torch.eye(int(obs.dim_topo))
        ).to(self.device)
        self.converter.last_topo = np.where(
            obs.topo_vect == -1, self.converter.last_topo, obs.topo_vect
        )
    ...
    def high_act(self, stacked_state, adj, sample=True):  # pi^h
        order, Q1, Q2 = None, 0, 0
        with torch.no_grad():
            # stacked_state last element onwards is topology and everything before that x # B, N, F
            stacked_t, stacked_x = stacked_state[..., -1:], stacked_state[..., :-1]
            emb_input = stacked_x
            state = self.emb(emb_input, adj).detach()
            actor_input = [state, stacked_t.squeeze(-1)]  # [x, tau topology]
            if sample:
    ...
    def update(self):
        ...
        self.Q.train()  # critic
        self.emb.train()  # graph embedding NN
        self.actor.eval()
        ...
        stacked_t, stacked_x = (
            stacked_states[..., -1:],
            stacked_states[..., :-1],
        )  # ? ... elipsis is really just : in all dimensions up til last
        ...
        with torch.no_grad():
            ...
            targets = (
                self.tQ.min_Q(tstates2, critic_input2, adj2, order2)
                - self.log_alpha.exp() * log_pi2
            )
    ...
    def load_model(self, path, name=None):
        ...
        self.actor.load_state_dict(
            torch.load(os.path.join(path, f"{head}actor.pt"), map_location=self.device)
        )
        self.emb.load_state_dict(
            torch.load(os.path.join(path, f"{head}emb.pt"), map_location=self.device)
        )
        self.Q.load_state_dict(
            torch.load(os.path.join(path, f"{head}Q.pt"), map_location=self.device)
        )
