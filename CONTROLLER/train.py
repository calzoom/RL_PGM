...
class TrainAgent(object):
    ...
    def interaction(self, obs, prev_act, cid, ffw, start_step):
        ...
        while True:
            ...
            step += 1
            if start_step + step == 864:  # can take up to 864 steps
                done = True
    ...
    def multi_step_transition(self, temp_memory):
        ...
        for tran in reversed(temp_memory):
            ...
            new_tran = (
                ...
                action,
                running_reward,
                final_state,  # ! this thing is "state2"
                final_adj,  # ! adj 2
                final_die,
                order,
            )
    ...
    def evaluate(self, chronics, max_ffw, fig_path, mode="best", plot_topo=False):
        ...
        for idx, i in enumerate(chronics):
            if max_ffw == 5:
                ...
                ffw = int(
                    np.argmin(
                        [
                            self.dn_ffw[(i, fw)][0]  # survived steps
                            for fw in range(max_ffw)
                            if (i, fw) in self.dn_ffw and self.dn_ffw[(i, fw)][0] >= 10
                        ]
                    )
                )
                # fastforward to the min num_steps survived by do_nothing on chronic i

            dn_step = self.dn_ffw[(i, ffw)][0]
            self.test_env.seed(59)  # 59
            self.test_env.set_id(i)
            obs = self.test_env.reset()
            self.agent.reset(obs)

            if ffw > 0:
                self.test_env.fast_forward_chronics(
                    ffw * 288 - 3
                )  # 288 increments of 5 min per day
                obs, *_ = self.test_env.step(self.test_env.action_space())
        ...
        if plot_topo:
            with open(
                os.path.join(fig_path, f"{mode}_{stats['score']:.3f}.txt"), "w"
            ) as f:
                f.write(str(stats))
                f.write(str(result))
