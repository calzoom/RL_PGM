...
class graphGoalConverter:
    ...
    def convert_obs(self, o):
        # o.shape : (B, O) grid2op gives me this
        # output (Batch, Node, Feature)
        length = self.action_space.dim_topo  # N = number elements in powergrid
        ...
        return state, topo_.unsqueeze(-1)  # state=X, topo=Tau
    ...
    def init_action_converter(self):
            ...
            if num_topo > self.mask and num_topo < self.mask_hi:
                self.sub_mask.extend(
                    [
                        j
                        for j in range(
                            self.sub_to_topo_begin[i] + 1, self.sub_to_topo_end[i]
                        )
                    ]
                )
                self.psubs.append(i)
        ...
        for i in range(self.obs_space.n_line):
            if (self.obs_space.line_or_to_subid[i] not in self.psubs) and (
                self.obs_space.line_ex_to_subid[i] not in self.psubs
            ):
    ...
    def plan_act(self, goal, topo_vect, sub_order_score=None):
        ...
        if sub_order_score is None:
            ...
            sub_order = [
                i[0]
                for i in sorted(
                    list(zip(self.masked_sorted_sub, sub_order_score.tolist())),
                    key=lambda x: -x[1],
                )
            ]
