...
class GATLayer(nn.Module):
    def __init__(self, output_dim, nheads, dropout=0):
        ...
        self.slf_attn = MultiHeadAttention(
            nheads, output_dim, output_dim // 4, dropout=dropout
        )
...
class Actor(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        return mu, log_std, state  # mu in R^n
    ...
    def sample(self, x, adj):
        ...
        if self.use_order:
            ...
            order_log_std = torch.clamp(
                order_log_std, self.log_std_min, self.log_std_max
            )
    ...
    def rsample(self, x, adj, eps=1e-5):
        ...
        if self.use_order:
            ...
            order = torch.tanh(order_z)
            order_log_pi = order_normal.log_prob(order_z) - torch.log(
                1 - order.pow(2) + eps
            )
...
class SoftQ(nn.Module):
    def __init__(
        ...
        self.down = nn.Linear(output_dim, 1)
        concat_dim = (
            int(node + action_dim + n_sub) if use_order else int(node + action_dim)
        )
        hidden_dim = concat_dim // 4
        self.out = nn.Linear(concat_dim, hidden_dim)
        self.out2 = nn.Linear(hidden_dim, 1)

    def forward(self, x, a, adj, order=None):  # a = g'
        x = self.gat1(x, adj)
...
class DoubleSoftQ(nn.Module):
    def __init__(
        ...
        self.Q1 = SoftQ(
            output_dim, nheads, node, action_dim, use_order, n_sub, dropout, init_w
        )
        self.Q2 = SoftQ(
            output_dim, nheads, node, action_dim, use_order, n_sub, dropout, init_w
        )
