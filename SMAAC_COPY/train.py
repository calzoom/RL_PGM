...
class TrainAgent(object):
    ...
    def compute_episode_score(self, chronic_id, agent_step, agent_reward, ffw=None):
        ...
        if ffw is None:
            ...
            ep_do_nothing_nodisc_reward = self.ep_infos[chronic_id][
                "donothing_nodisc_reward"
            ]
            ...
            (
                ep_dn_played,
                ep_do_nothing_reward,
                ep_do_nothing_nodisc_reward,
            ) = self.dn_ffw[(chronic_id, ffw)]
            ep_loads = np.array(self.ep_infos[chronic_id]["sum_loads"])[
                start_idx:end_idx
            ]
    ...
    def interaction(self, obs, prev_act, cid, ffw, start_step):
        ...
        while True:
            ...
            if prev_act:  # ! from 102 originally
                act = prev_act
                prev_act = None
            else:
    ...
    def train(
        ...
        if joint:  # ! new
            ...
            assert opp_train_steps > 0
        best_score = -100

        # initialize training chronic sampling weights
        train_chronics_ffw = [
            (cid, fw) for cid in train_chronics for fw in range(max_ffw)
        ]
        ...
        while self.agent.update_step < nb_frame:
            ...
            if self.opponent:  # ! new
                self.opponent.reset(obs)
                ...
                if len(self.agent.memory) > self.agent.update_start:
                    ...
                    if joint and self.agent.update_step % test_step == 0:
                        ...
                        if ffw > 0:
                            ...
                            obs, *_ = self.env.step(self.env.action_space())
                        self.agent.reset(obs)
                        self.opponent.reset(obs)  # ! new
                        self.opponent.learn(opp_train_steps)
    ...
    def test(self, chronics, max_ffw, f=None, verbose=False):
        ...
        for idx, i in enumerate(chronics):
                ...
                ffw = int(
                    np.argmin(
                        [
                            ...
                            if (i, fw) in self.dn_ffw and self.dn_ffw[(i, fw)][0] >= 10
                        ]
                    )
                )
            ...
            l2rpn_score = float(
                self.compute_episode_score(i, alive_frame, total_reward, ffw)
            )
            print(
                f"[Test Ch{i:4d}({ffw:2d})] {alive_frame:3d}/864 ({dn_step:3d}) Score: {l2rpn_score:9.4f} "
            )
    ...
    def evaluate(self, chronics, max_ffw, fig_path, mode="best", plot_topo=False):
        ...
        for idx, i in enumerate(chronics):
            ...
            dn_step = self.dn_ffw[(i, ffw)][0]
            self.test_env.seed(59)
            self.test_env.opponent.rng = np.random.default_rng(seed=59)  # ! new
            self.test_env.set_id(i)
            obs = self.test_env.reset()
            self.agent.reset(obs)
            if self.opponent:  # ! new
                self.opponent.reset(obs)
                self.opponent._next_attack_time = 1
            ...
            while not done:
                ...
                act = self.agent.act(obs, 0, 0)
                obs, reward, done, info = self.test_env.step(act)
                if info["is_illegal"] or info["is_ambiguous"]:  # ! btwn 413 and 414
                    if done:
                        break
                    continue
                ...
                if self.opponent:
                    if self.opponent.remaining_time >= 0:
                        ...
                        if attack is not None:
                            ...
                            obs.time_before_cooldown_line[
                                self.opponent.attack_line
                            ] = self.opponent.remaining_time  # ! end of opp
                    ...
                    if bus_goal is not None:
                        ...
                        if self.agent.is_safe(obs) and len(self.agent.low_actions) == 0:
                            if (
                                np.sum(
                                    [
                                        a == self.test_env.action_space()
                                        for a in temp_acts
                                    ]
                                )
                                < len(temp_acts) - 1
                            ) and alive_frame - prev_step > 1:
                                ...
                                print(
                                    prev_step,
                                    alive_frame - prev_step,
                                    (prev_topo != temp_topo).sum(),
                                )
                            bus_goal = None
                            temp_acts = []
            ...
            if plot_topo:
                ...
                np.save(
                    os.path.join(fig_path, f"{mode}_{idx}_topo.npy"),
                    np.array(topo_dist),
                )
        ...
        with open(fig_path + "{mode}_{stats['score']:.3f}.txt", "w") as f:
            ...
            f.write(str(result))
        print(
            f"Mean/std score: {np.mean(np.array(scores)):9.4f} +/- {np.std(np.array(scores)):9.4f}"
        )
        print(
            f"Mean/std steps: {np.mean(np.array(steps)):9.4f} +/- {np.std(np.array(steps)):9.4f}"
        )
