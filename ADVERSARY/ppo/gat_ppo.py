...
class GPPO:
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps:  # ALG STEP 2
            ...
            if i_so_far % self.save_freq == 0:
                torch.save(self.actor.state_dict(), "./result/ppo_actor_" + self.name + ".pth")
                torch.save(
                    self.critic.state_dict(), "./result/ppo_critic_" + self.name + ".pth"
                )
    ...
    def rollout(self):
        ...
        while t < self.timesteps_per_batch:
            ep_rews = []  # rewards collected per episode (play with controller til gameover)
            ...
            for ep_t in range(self.max_timesteps_per_episode):
                ...
                action, log_prob = self.get_action(state, adj)
        ...
        batch_adjs = torch.cat(batch_adjs, dim=0)
        ...
        return batch_obs, batch_adjs, batch_acts, batch_log_probs, batch_rtgs, batch_lens
    ...
    def tell_attack_continues(
        self, observation=None, agent_action=None, env_actio=None, budget=None
    ):
        self._next_attack_time = None
    ...
    def act(self, obs, reward, done):
        ...
        if self._next_attack_time is None:
            self._next_attack_time = (
                self._attack_period
            )  # 1 + np.random.randint(self._attack_period)
    ...
    def evaluate(self, batch_obs, batch_adjs, batch_acts):
        """
    ...
    def convert_obs(self, o):
        ...
        if self.transfer:
            li_vect = []
            for el in o.attr_list_vect:
                v = o._get_array_from_attr_name(el).astype(np.float32)
            ...
            return torch.tensor([np.concatenate(li_vect)]).to(self.device)
        ...
        danger = (
            (o[..., self.rho] >= self.danger - 0.05)
            & self.thermal_limit_under400.to(o.device)
    ...
    def _init_hyperparameters(self, hyperparameters):
        ...
        self.n_updates_per_iteration = (
            10  # Number of times to update actor/critic per iteration
        )
        self.lr = 0.0005  # Learning rate of actor optimizer
        self.gamma = (
            0.99  # Discount factor to be applied when calculating Rewards-To-Go
        )
    ...
    def _init_attacks(self, hyperparameters):
        ...
        self._next_attack_time = None
        self._attack_period = hyperparameters["attack_period"]
    ...
    def _init_obs_converter(self, hyperparameters, state_mean, state_std):
        ...
        self.subs = [
            {"e": [], "o": [], "g": [], "l": []} for _ in range(self.action_space.n_sub)
        ]
    ...
    def _log_summary(self):
        ...
        avg_actor_loss = np.mean(
            [losses.float().cpu().mean() for losses in self.logger["actor_losses"]]
        )
